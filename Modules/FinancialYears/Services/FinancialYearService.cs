    ...
    ILogger<FinancialYearService> logger) : IFinancialYearService
{
    ...
    public async Task CreateAsync(DateTime startDate, CancellationToken ct = default)
    {
        ...
        logger.LogInformation("Financial year '{Name}' created ({Start} to {End})",
            name, regional.FormatDate(startDate), regional.FormatDate(endDate));
    }
    ...
    public async Task EnsureCurrentYearAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            // Read configured FY start month from firm settings (default: April)
            var config = await context.AppConfigs.FirstOrDefaultAsync(ct).ConfigureAwait(false);
            var fyMonth = config?.FinancialYearStartMonth ?? 4;

            // Determine current FY start based on configured month
            var fyStart = now.Month >= fyMonth
                ? new DateTime(now.Year, fyMonth, 1)
                : new DateTime(now.Year - 1, fyMonth, 1);
