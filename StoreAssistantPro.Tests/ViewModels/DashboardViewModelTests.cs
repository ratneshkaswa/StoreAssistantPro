...
public class DashboardViewModelTests
{
    ...
    private DashboardViewModel CreateSut() => new(_dashboardService);

    [Fact]
    public async Task LoadDashboard_SetsTotalProducts()
    {
        ...
        var sut = CreateSut();
        await sut.LoadDashboardCommand.ExecuteAsync(null);

        Assert.Equal(3, sut.TotalProducts);
    }

    [Fact]
    public async Task LoadDashboard_CountsLowStockProducts()
    {
        _dashboardService.GetSummaryAsync().Returns(new DashboardSummary(
            TotalProducts: 4,
            LowStockCount: 3,
    ...
    [Fact]
    public async Task LoadDashboard_CalculatesTodaysSalesTotal()
    {
        _dashboardService.GetSummaryAsync().Returns(new DashboardSummary(
            TotalProducts: 0,
            LowStockCount: 0,
    ...
    [Fact]
    public async Task LoadDashboard_WithNoData_AllValuesAreZero()
    {
        _dashboardService.GetSummaryAsync().Returns(new DashboardSummary(
            TotalProducts: 0,
            LowStockCount: 0,
